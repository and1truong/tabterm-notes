...
export function CwdPickerModal({
  ...
  const load = async (path: string) => {
    setError(null);
    try {
      const res = await fetch(`/api/modules/files/r/ls?path=${encodeURIComponent(path)}`);
    ...
    >
      ...
      >
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-2 space-y-0.5">
          {error && (
            <Notice variant="error" layout="inline" className="text-xs px-3 py-3">{error}</Notice>
          )}
        ...
        <div className="flex items-center gap-2 px-4 h-12 border-t border-[var(--border)]">
          ...
          <button
            onClick={commit}
            disabled={!data}
            className="px-3 h-7 text-xs rounded-md bg-[var(--accent)] text-[var(--panel)] hover:opacity-90 disabled:opacity-40"
          >
            Select
          </button>
        </div>
